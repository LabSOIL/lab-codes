...
def matlab_integral_data():
    ...
    df = pd.read_csv(os.path.join(RESOURCES_DIR, "matlab_integral_output.csv"))
...
@pytest.fixture(scope="session")
def example_input_data_8col():
    """Read example input data"""

    df = pd.read_csv(
        os.path.join(RESOURCES_DIR, "Example_input_8col.txt"), header=25
    )
...
@pytest.fixture(scope="session")
def example_input_data_6col():
    """Read example input data"""

    df = pd.read_csv(
        os.path.join(RESOURCES_DIR, "Example_input_6col.txt"), header=62
    )
    # df.set_index('Time/s', inplace=True)  # Set index to the time column
    df.columns = df.columns.str.strip()  # Strip whitespace from the columns
